...
void GpuWavesApp::DrawScene()
{
    ...
    for(UINT p = 0; p < techDesc.Passes; ++p)
    {
		...
		// Set per object constants.
		DirectX::XMMATRIX world = XMLoadFloat4x4(&mWavesWorld);
		DirectX::XMMATRIX worldInvTranspose = MathHelper::InverseTranspose(world);
		DirectX::XMMATRIX worldViewProj = world*view*proj;
		...
		Effects::WaveRenderFX->SetDisplacementTexelSize(DirectX::XMFLOAT2(1.0f/mWaves.ColumnCount(), 1.0f/mWaves.RowCount()));
...
DirectX::XMFLOAT3 GpuWavesApp::GetHillNormal(float x, float z)const
{
	// n = (-df/dx, 1, -df/dz)
	DirectX::XMFLOAT3 n(
